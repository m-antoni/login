			...
			<form method="POST" action="{{ route('register.store') }}">
				@csrf
					<div class="form-group row">
					   	<div class="col-md-3">
					        <label>First:</label>
					        <input type="text" name="first" class="form-control{{ $errors->has('first') ? ' is-invalid' : '' }}"  value="{{ old('first')}}">
					        @if ($errors->has('first'))
					    ...
					    <div class="col-md-3">
					        <label>Last:</label>
					        <input type="text" name="last" class="form-control{{ $errors->has('last') ? ' is-invalid' : '' }}" value="{{ old('last') }}">
					        @if ($errors->has('last'))
					            <span class="text-danger" role="alert">
					                {{ $errors->first('last') }}
					            </span>
					...
					<div class="form-group row">
					        ...
					        <input type="email" name="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email')}}">
					        @if ($errors->has('email'))
					            <span class="text-danger" role="alert">
					                {{ $errors->first('email') }}
					            </span>
					        ...
					        <select name="user_type" class="form-control{{ $errors->has('user_type') ? ' is-invalid' : '' }}">
					        	<option value="" selected disabled>Choose here...</option>
							      ...
							      <option value="Intern" {{old('user_type') == 'Intern' ? 'selected' : ''}}>Intern</option>
							    </select>
					...
					<div class="form-group row">
					    ...
					    <div class="col-md-4">
					        ...
					        <select name="department" class="form-control{{ $errors->has('department') ? ' is-invalid' : '' }}">
					        	<option value="" selected disabled>Choose here...</option>
					        	@foreach($department as $departmentOptionKey => $departmentOptionValue)
					        			<option value="{{$departmentOptionKey}}" {{old('department') == $departmentOptionKey ? 'selected' : ''}}>{{$departmentOptionValue}}</option>
					        	@endforeach
							    </select>
				    	...
				    	<div class="col-md-4">	
						     <label>Date Hired:</label>
						        <input type="date" name="date_hired" class="form-control{{ $errors->has('date_hired') ? ' is-invalid' : '' }}" value="{{ old('date_hired')}}">
					...
					<div class="form-group row">
					        ...
					        <input type="text" name="id_number" class="form-control{{ $errors->has('id_number') ? ' is-invalid' : '' }}" name="id_number" value="{{ old('id_number')}}">
